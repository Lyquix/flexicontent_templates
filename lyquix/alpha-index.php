...
if ($caching)
	$selected_letter = JRequest::getVar('letter', '', 'get');
else
	$selected_letter = JRequest::getVar('letter', '', 'post');
?>
...
<?php if($flag) {
?>
<a class="btn btn-medium fc_alpha_index" href="javascript:;" onclick="document.getElementById('alpha_index').value='';document.getElementById('adminForm').submit();"><?php echo JText::_('FLEXI_ALL'); ?><
/a>
<?php $flag = false;
	}
?>
...
foreach ($letters as $letter) :
	...
	if ($alphacharsep)
		$aiclass = "btn btn-medium fc_alpha_index_sep";
	else
		$aiclass = "btn btn-medium fc_alpha_index";
	$currentclass = '';
	if ($letter_label == $selected_letter) {
		$currentclass = 'current active';
	}
	if ($has_item) :
		if ($alphacharsep && !$group_start)
			echo "<span class=\"fc_alpha_index_sep\">$alphacharsep</span>";
